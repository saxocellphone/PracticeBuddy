...
} from "@core/arpeggio/sequence.ts";
  ...
  try {
    ...
    return {
      ...
      micSensitivity:
        ...
        parsed.micSensitivity <= 100
          ? parsed.micSensitivity
          : DEFAULT_SETTINGS.micSensitivity,
      selectedDeviceId:
      ...
      scaleStartPosition: [
        ...
        "next-measure",
        "immediately",
      ].includes(parsed.scaleStartPosition)
        ? parsed.scaleStartPosition
        : DEFAULT_SETTINGS.scaleStartPosition,
      instrumentId:
...
function MainApp() {
  ...
  const [noteDuration, setNoteDuration] =
    useState<NoteDuration>(loadNoteDuration);
  ...
  const [metronomeEnabled, setMetronomeEnabled] = useState(
    saved.metronomeEnabled,
  );
  const [scaleStartPosition, setScaleStartPosition] =
  ...
  const {
    ...
    startScalePractice,
    stopScalePractice,
    processFrame: scaleProcessFrame,
    skipNote,
  } = useScalePractice();
  ...
  const {
    ...
    onBeatSubscribe: metronome.onBeat,
  });
  ...
  useEffect(() => {
    if (view !== "practicing" || !pitchResult.pitch || timingMode !== "rhythm")
      return;
  ...
  const {
    ...
    switchDevice: audioSwitchDevice,
    currentDeviceId,
  } = audio;
  ...
  const handleRetry = useCallback(() => {
    ...
    if (timingMode === "rhythm") {
      ...
      if (activeMode === "walking-bass") {
        ...
        const adaptedSequence = arpeggioToScaleSequence(
          arpeggioSequenceRef.current,
          ignoreOctave,
        );
  ...
  const handleStartRhythmPractice = useCallback(
    async (sequence: ScaleSequence) => {
  ...
  const handleStartArpeggioPractice = useCallback(
    async (sequence: ArpeggioSequence) => {
      let ctx = audioCtx;
      if (audioState === "uninitialized") {
        ctx = await audioInitialize(saved.selectedDeviceId ?? undefined);
      }
  ...
  const handleMetronomeToggle = useCallback(() => {
    if (metronomeIsPlaying) {
      ...
      if (audioState === "uninitialized") {
        audioInitialize(saved.selectedDeviceId ?? undefined).then(() =>
          metronomeStart(),
        );
      } else {
        metronomeStart();
      ...
      <AdvancedSettings>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
          ...
          <select
            ...
            onChange={(e) =>
              setScaleStartPosition(e.target.value as ScaleStartPosition)
            }
            style={{
              padding: "6px 10px",
          ...
          >
            {(
              Object.entries(SCALE_START_LABELS) as [
                ScaleStartPosition,
                string,
              ][]
            ).map(([value, label]) => (
  ...
  return (
    ...
    >
      ...
      {view === "practicing" &&
        ...
        timingMode === "follow" && (
          <>
            {scaleState && <ScaleBanner scaleState={scaleState} />}
